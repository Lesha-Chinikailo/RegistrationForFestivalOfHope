...
def connect_to_supabase():
    ...
    try:
        if not SUPABASE_URL or not SUPABASE_KEY:
            st.error("❌ Настройте SUPABASE_URL и SUPABASE_KEY в файле .env!")
            return None
...
# JavaScript для форматирования телефона в реальном времени
st.markdown("""
<script>
function formatBelarusPhone(input) {
    ...
    if (digits.length === 11 && digits.startsWith('8')) {
        number = '375' + digits.substring(1);
    }
    // Если 10 цифр и не начинается с 375
    else if (digits.length === 10 && !digits.startsWith('375')) {
        number = '375' + digits;
    }
    // Если 9 цифр
    ...
    // Если 11 цифр и начинается с 375
    else if (digits.length === 11 && digits.startsWith('375')) {
        number = digits;
    }
    else {
        number = digits;
    ...
    if (number.length === 12 && number.startsWith('375')) {
        return `+${number.substring(0,3)} (${number.substring(3,5)}) ${number.substring(5,8)}-${number.substring(8,10)}-${number.substring(10,12)}`;
    }
    else if (number.length === 11 && number.startsWith('375')) {
        return `+${number.substring(0,3)} (${number.substring(3,5)}) ${number.substring(5,8)}-${number.substring(8,10)}-${number.substring(10,11)}`;
    }

    return input;
}

// Функция для применения форматирования ко всем полям телефона
function setupPhoneFormatting() {
    // Находим все поля ввода телефона
    const phoneInputs = document.querySelectorAll('input[type="text"]');

    phoneInputs.forEach(input => {
        // Проверяем, является ли поле полем для телефона (по плейсхолдеру или id)
        if (input.placeholder && (input.placeholder.includes('телефон') || input.placeholder.includes('+375'))) {
            // Сохраняем оригинальное событие
            const originalInput = input.oninput;

            // Добавляем обработчик события ввода
            input.oninput = function(e) {
                const cursorPosition = this.selectionStart;
                const formatted = formatBelarusPhone(this.value);
                if (formatted !== this.value) {
                    this.value = formatted;
                    // Восстанавливаем позицию курсора
                    this.setSelectionRange(cursorPosition, cursorPosition);
                }
                if (originalInput) originalInput(e);
            };
        }
    });
}
...
setTimeout(setupPhoneFormatting, 1000);
...
if supabase:
    ...
    with tab1:
        ...
        st.caption("📱 Номер автоматически форматируется при вводе: 80291234567 → +375 (29) 123-45-67")
        ...
        with st.form("add_user", clear_on_submit=True):
            ...
            # Поле для телефона с JavaScript форматированием
            phone = st.text_input(
                "Номер телефона *",
            ...
            if submitted:
                if phone:
                    # Удаляем все нецифровые символы для сохранения в БД
                    clean_phone = re.sub(r'\D', '', phone)
                    success, result = add_participant(supabase, f, i, o, clean_phone, day)
                    if success:
                        if f and i:
    ...
    with tab2:
        ...
        if participants:
            ...
            df['phone_display'] = df['phone'].apply(format_phone_display)
    ...
    # --- ВКЛАДКА 3: РЕДАКТИРОВАНИЕ И УДАЛЕНИЕ ---
    with tab3:
        ...
        if participants:
            ...
            # Форматируем телефон для отображения в форме
            selected_phone_display = selected.get('phone', '')
            digits = re.sub(r'\D', '', str(selected_phone_display))
            if len(digits) == 12 and digits.startswith('375'):
                selected_phone_display = f"+{digits[:3]} ({digits[3:5]}) {digits[5:8]}-{digits[8:10]}-{digits[10:12]}"
                ...
                if save_button:
                    if edit_phone:
                        ...
                        success, result = update_participant(
                            ...
                            edit_middle_name, clean_phone, edit_date
                        )
                        if success:
    ...
    with tab4:
        ...
        if participants:
            ...
            df['phone_formatted'] = df['phone'].apply(format_phone_export)
